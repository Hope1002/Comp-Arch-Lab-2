...
bool interpret(char* instr){
		...
		if((op = operation(tokenTemp, (temp = add)))){//ADD
			...
			printf(" new adress: %d, offset: %d, base register: %d \n", newaddress, offset, baseRegister);
...
/*This function will return only a char* of only digits*/
char* tokenizeHelper(char **token){
...
bool operation(char** tokens, char* compare){
	...
	//char *tempTok = compare[0];
	//printf("i am in operation function \n ");
	printf("%c %C \n", tempToken[0], compare[0] );
